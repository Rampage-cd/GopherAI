...
//4.支持同步与流式两种生成方式
import (
	"GopherAI/common/rabbitmq" //消息异步存储实现
	"GopherAI/model"           //业务层消息结构
...
type AIHelper struct {
	model    AIModel          //接口类型，OpenAIModel和OllamaModel都实现了该接口
	messages []*model.Message //会话中的历史消息
	mu       sync.RWMutex
	//一个会话绑定一个AIHelper
	SessionID string
	//通过函数指针解耦存储实现，避免循环依赖（可以是数据库，MQ，也可以是同步或异步）
